...
import { createBlobService, createQueueService, createTableService, TableQuery, TableUtilities } from 'azure-storage';
...
app.get('/', (req, res) => {
    ...
    tableService.queryEntities('reviews', query, null, (error, result, response) => {
        if (!error) {
            const reviews = result.entries.sort((a, b) => new Date(b.Timestamp._) - new Date(a.Timestamp._));
...
app.post('/post', upload.single('image'), (req, res) => {
    ...
    const blobName = `${Date.now()}-${req.file ? req.file.originalname : 'noimage'}`;

    if (imagePath) {
        // Upload the image to Blob Storage
        blobService.createBlockBlobFromLocalFile('images', blobName, imagePath, (error, result, response) => {
            if (!error) {
                // Add the image to the queue for processing
                queueService.createMessage('resize-queue', JSON.stringify({ blobName }), (error, result, response) => {
                    if (!error) {
                        // Save the review to Table Storage
                        saveReview(username, reviewText, blobName, blobService.getUrl('images', blobName), '', res);
                    } else {
                        res.status(500).send(error);
                    }
                });
...
function saveReview(username, reviewText, blobName, imageUrl, thumbnailUrl, res) {
