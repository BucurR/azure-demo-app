...
function processQueue() {
  queueService.getMessages(
    "resize-queue",
    { numofmessages: 1, visibilitytimeout: 30 },
                  ...
                  blobService.createBlockBlobFromLocalFile(
                    ...
                    (error, result, response) => {
                      if (!error) {
                        ...
                        const thumbnailUrl = generateSASToken(
                          "thumbnails",
                          thumbnailName
                        );
                        ...
                        tableService.mergeEntity(
                          ...
                          (error, result, response) => {
                            if (!error) {
                              console.log(
                                `Updated table storage with thumbnail URL: ${thumbnailUrl}`
                              );
                              ...
                              queueService.deleteMessage(
                                ...
                                (error, response) => {
                                  if (!error) {
                                    console.log(
                                      `Deleted message from queue for blob: ${blobName}`
                                    );
                                    fs.unlink(downloadPath, (err) => {
                                      if (err) {
                                        console.error(
                                          `Failed to delete downloaded image: ${err}`
                                        );
                                      } else {
                                        console.log(
                                          `Deleted downloaded image: ${downloadPath}`
                                        );
                                      }
                                    });
                                  } else {
                                    console.error(
                                      `Failed to delete message from queue: ${error}`
                                    );
                                  }
                                }
                              );
                              ...
                              console.error(
                                `Failed to update table storage: ${error}`
                              );
                            }
                          }
                        );
